...
function BoardCard(props) {
  ...
  const cardCoverImage = _.find(card.attachments, {
    id: card.attachmentCoverId,
  });
          ...
          >
            ...
            <div className="p-16 pb-0">
              {card.labels.length > 0 && (
                <div className="flex flex-wrap mb-8 -mx-4">
                  {card.labels.map((id, idx) => (
                    <BoardCardLabel id={id} key={idx} />
                  ))}
                </div>
              )}
            ...
            <div className="flex justify-between h-48 px-16">
              <div className="flex items-center space-x-4">
                {card.subscribed && (
                  <FuseSvgIcon size={16} color="action" icon={Icons.faEye} />
                )}

                {card.description !== "" && (
                  <FuseSvgIcon size={16} color="action" icon={Icons.faFile} />
                )}
              ...
              <div className="flex items-center justify-end space-x-12">
                {card.memberIds.length > 0 && (
                  <div className="flex justify-start">
                    ...
                    >
                      {card.memberIds.map((id, idx) => {
                        const member = _.find(members, { id });
                        return (
                          <Tooltip title={member.name} key={idx}>
                            <Avatar
                              key={index}
                              alt="member"
                              src={member.avatar}
